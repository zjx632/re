...
#define NEDMALLOC_TESTLOGENTRY(tc, np, type, mspaceex, size, mem, alignment, flags, returned) (((type)&ENABLE_LOGGING)&&((size)>16*1024))
...
#define USE_RECURSIVE_LOCKS 1
 ...
 #define USE_LOCKS 1
#endif
#define FOOTERS 1           /* Need to enable footers so frees lock the right mspaceex */
...
#define NEDMALLOC_TESTLOGENTRY(tc, np, type, mspaceex, size, mem, alignment, flags, returned) ((type)&ENABLE_LOGGING)
#endif
...
static size_t mspaceexcounter=(size_t) 0xdeadbeef;
#endif
...
static FORCEINLINE NEDMALLOCNOALIASATTR NEDMALLOCPTRATTR void *CallMalloc(void *RESTRICT mspaceex, size_t size, size_t alignment, unsigned flags) THROWSPEC
{
	void *RESTRICT ret=0;
	...
	ret=mspaceex_malloc2((mstate) mspaceex, size, alignment, flags);
#ifdef HAVE_VALGRIND
	if(ret) VALGRIND_MEMPOOL_ALLOC(mspaceex, ret, size);
#endif
#ifndef ENABLE_FAST_HEAP_DETECTION
	if(ret)
	{
		...
		if(!leastusedaddress || (void *)((mstate) mspaceex)->least_addr<leastusedaddress) leastusedaddress=(void *)((mstate) mspaceex)->least_addr;
	...
	_ret[0]=(size_t) mspaceex;
	_ret[1]=size-3*sizeof(size_t);
#endif
	return ret;
}

static FORCEINLINE NEDMALLOCNOALIASATTR NEDMALLOCPTRATTR void *CallRealloc(void *RESTRICT mspaceex, void *RESTRICT mem, int isforeign, size_t oldsize, size_t newsize, size_t alignment, unsigned flags) THROWSPEC
{
	void *RESTRICT ret=0;
#if USE_MAGIC_HEADERS
	mstate oldmspaceex=0;
...
#endif
		if((ret=CallMalloc(mspaceex, newsize, alignment, flags)))
		{
#if defined(DEBUG)
...
#elif USE_ALLOCATOR==1
	ret=mspaceex_realloc2((mstate) mspaceex, mem, newsize, alignment, flags);
#ifdef HAVE_VALGRIND
	if(ret) VALGRIND_MEMPOOL_CHANGE(mspaceex, mem, ret, newsize);
#endif
#ifndef ENABLE_FAST_HEAP_DETECTION
	if(ret)
	{
...
static FORCEINLINE void CallFree(void *RESTRICT mspaceex, void *RESTRICT mem, int isforeign) THROWSPEC
{
#if USE_MAGIC_HEADERS
	mstate oldmspaceex=0;
...
#ifdef HAVE_VALGRIND
	{
		void *m=mspaceex ? mspaceex : get_mstate_for(mem2chunk(mem));
		mspaceex_free((mstate) mspaceex, mem);
...
typedef struct threadcache_t
{
	...
	unsigned int magic1;
#endif
	int mymspaceex;						/* Last mspaceex entry this thread used */
	long threadid;
...
struct nedpool_t
{
	...
	TLSVAR mycache;						/* Thread cache for this thread. 0 for unset, negative for use mspaceex-1 directly, otherwise is cache-1 */
	mstate m[MAXTHREADSINPOOL+1];		/* mspaceex entries for this pool */
};
...
#endif
#endif
static FORCEINLINE logentry *LogOperation(threadcache *tc, nedpool *np, LogEntryType type, int mspaceex, size_t size, void *mem, size_t alignment, unsigned flags, void *returned) THROWSPEC
{
#if ENABLE_LOGGING
	if(tc->logentries && NEDMALLOC_TESTLOGENTRY(tc, np, type, mspaceex, size, mem, alignment, flags, returned))
	{
		logentry *le;
	...
	if(p->caches)
	{
		...
		if(haslogentries)
		{
			...
			if(oh)
			{
				...
				fgetpos(oh, &pos2);
				if(pos1==pos2)
					fprintf(oh, "Timestamp, Pool, Operation, mspaceex, Size, Block, Alignment, Flags, Returned,\"Stack Backtrace\"\n");
				for(n=0; n<THREADCACHEMAXCACHES; n++)
				{
					if((tc=p->caches[n]) && tc->logentries)
					{
						...
						for(le=tc->logentries; le<tc->logentriesptr; le++)
						{
							...
							fprintf(oh, "%llu, 0x%p, %s, %d, %Iu, 0x%p, %Iu, 0x%x, 0x%p,\"%s\"\n",
								le->timestamp, le->np, LogEntryTypeString, le->mspaceex, le->size, le->mem, le->alignment, le->flags, le->returned, stackbacktrace);
						}
...
static NOINLINE threadcache *AllocCache(nedpool *RESTRICT p) THROWSPEC
{
	...
	tc->mymspaceex=abs(tc->threadid) % end;
#if ENABLE_LOGGING
	{
		mchunkptr cp;
...
static NOINLINE void ReleaseFreeInCache(nedpool *RESTRICT p, threadcache *RESTRICT tc, int mymspaceex) THROWSPEC
{
	unsigned int age=THREADCACHEMAXFREESPACE/8192;
#if USE_LOCKS
	/*ACQUIRE_LOCK(&p->m[mymspaceex]->mutex);*/
#endif
...
#if USE_LOCKS
	/*RELEASE_LOCK(&p->m[mymspaceex]->mutex);*/
#endif
}
static void threadcache_free(nedpool *RESTRICT p, threadcache *RESTRICT tc, int mymspaceex, void *RESTRICT mem, size_t size, int isforeign) THROWSPEC
{
	unsigned int bestsize;
	unsigned int idx=size2binidx(size);
	...
	*binsptr=tck;
	...
	if(tc->freeInCache>=THREADCACHEMAXFREESPACE)
		ReleaseFreeInCache(p, tc, mymspaceex);
#endif
}
...
#if USE_ALLOCATOR==0
	p->m[0]=(mstate) mspaceexcounter++;
#elif USE_ALLOCATOR==1
	if(!(p->m[0]=(mstate) create_mspaceex(capacity, 1))) goto err;
#ifdef HAVE_VALGRIND
...
static NOINLINE mstate Findmspaceex(nedpool *RESTRICT p, threadcache *RESTRICT tc, int *RESTRICT lastUsed, size_t size) THROWSPEC
{	/* Gets called when thread's last used mspaceex is in use. The strategy
	is to run through the list of all available mspaceexs looking for an
	...
	if(end<p->threads)
	{
		mstate temp;
#if USE_ALLOCATOR==0
		temp=(mstate) mspaceexcounter++;
#elif USE_ALLOCATOR==1
		if(!(temp=(mstate) create_mspaceex(size, 1)))
			goto badexit;
#endif
...
void nedtrimthreadcache(nedpool *p, int disable) THROWSPEC
{
	...
	{	/* Set to last used mspaceex */
		threadcache *tc=p->caches[mycache-1];
#if defined(DEBUG)
		printf("Threadcache utilisation: %lf%% in cache with %lf%% lost to other threads\n",
			100.0*tc->successes/tc->mallocs, 100.0*((double) tc->mallocs-tc->frees)/tc->mallocs);
#endif
		if(disable && TLSSET(p->mycache, (void *)(size_t)(-tc->mymspaceex))) abort();
		tc->frees++;
		RemoveCacheEntries(p, tc, 0);
		assert(!tc->freeInCache);
		if(disable)
...
#define GETmspaceex(m,p,tc,ms,s,action)                 \
  do                                                  \
  {                                                   \
    mstate m = Getmspaceex((p),(tc),(ms),(s));          \
    action;                                           \
...
static FORCEINLINE mstate Getmspaceex(nedpool *RESTRICT p, threadcache *RESTRICT tc, int mymspaceex, size_t size) THROWSPEC
{	/* Returns a locked and ready for use mspaceex */
	mstate m=p->m[mymspaceex];
	assert(m);
#if USE_LOCKS && USE_ALLOCATOR==1
	if(!TRY_LOCK(&p->m[mymspaceex]->mutex)) m=Findmspaceex(p, tc, &mymspaceex, size);
	/*assert(IS_LOCKED(&p->m[mymspaceex]->mutex));*/
#endif
	return m;
}
...
static NOINLINE void GetThreadCache_cold2(nedpool *RESTRICT *RESTRICT p, threadcache *RESTRICT *RESTRICT tc, int *RESTRICT mymspaceex, int mycache) THROWSPEC
{
	if(!mycache)
	{	/* Need to allocate a new cache */
		*tc=AllocCache(*p);
		...
		{	/* Disable */
			...
			*mymspaceex=(*tc)->mymspaceex;
	}
	else
	{	/* Cache disabled, but we do have an assigned thread pool */
		*tc=0;
		*mymspaceex=-mycache-1;
	}
}
static FORCEINLINE void GetThreadCache(nedpool *RESTRICT *RESTRICT p, threadcache *RESTRICT *RESTRICT tc, int *RESTRICT mymspaceex, size_t *RESTRICT size) THROWSPEC
{
	int mycache;
#if THREADCACHEMAX
	...
	else GetThreadCache_cold2(p, tc, mymspaceex, mycache);
	assert(*mymspaceex>=0);
#if USE_LOCKS
...
NEDMALLOCNOALIASATTR NEDMALLOCPTRATTR void * nedpmalloc2(nedpool *p, size_t size, size_t alignment, unsigned flags) THROWSPEC
{
	...
	threadcache *tc;
	int mymspaceex;
	GetThreadCache(&p, &tc, &mymspaceex, &size);
#if THREADCACHEMAX
	...
	{	/* Use the thread cache */
		if((ret=threadcache_malloc(p, tc, &size)))
		{
			if((flags & M2_ZERO_MEMORY))
				memset(ret, 0, size);
			LogOperation(tc, p, LOGENTRY_THREADCACHE_MALLOC, mymspaceex, size, 0, alignment, flags, ret);
		}
	}
#endif
	if(!ret)
	{	/* Use this thread's mspaceex */
        GETmspaceex(m, p, tc, mymspaceex, size,
                  ret=CallMalloc(m, size, alignment, flags));
		if(ret)
			LogOperation(tc, p, LOGENTRY_POOL_MALLOC, mymspaceex, size, 0, alignment, flags, ret);
	}
	LogOperation(tc, p, LOGENTRY_MALLOC, mymspaceex, size, 0, alignment, flags, ret);
	return ret;
}
NEDMALLOCNOALIASATTR NEDMALLOCPTRATTR void * nedprealloc2(nedpool *p, void *mem, size_t size, size_t alignment, unsigned flags) THROWSPEC
{
	...
	{	/* Use the thread cache */
		if((ret=threadcache_malloc(p, tc, &size)))
		{
			...
			LogOperation(tc, p, LOGENTRY_THREADCACHE_MALLOC, mymspaceex, size, mem, alignment, flags, ret);
			if(!isforeign && memsize>=sizeof(threadcacheblk) && memsize<=(THREADCACHEMAX+CHUNK_OVERHEAD))
			{
				threadcache_free(p, tc, mymspaceex, mem, memsize, isforeign);
				LogOperation(tc, p, LOGENTRY_THREADCACHE_FREE, mymspaceex, memsize, mem, 0, 0, 0);
			}
			else
			{
				CallFree(0, mem, isforeign);
				LogOperation(tc, p, LOGENTRY_POOL_FREE, mymspaceex, memsize, mem, 0, 0, 0);
			}
		}
	}
#endif
	if(!ret)
	{	/* Reallocs always happen in the mspaceex they happened in, so skip
		locking the preferred mspaceex for this thread */
		ret=CallRealloc(p->m[mymspaceex], mem, isforeign, memsize, size, alignment, flags);
		if(ret)
			LogOperation(tc, p, LOGENTRY_POOL_REALLOC, mymspaceex, size, mem, alignment, flags, ret);
	}
	LogOperation(tc, p, LOGENTRY_REALLOC, mymspaceex, size, mem, alignment, flags, ret);
	return ret;
}
NEDMALLOCNOALIASATTR void   nedpfree2(nedpool *p, void *mem, unsigned flags) THROWSPEC
{	/* Frees always happen in the mspaceex they happened in, so skip
	locking the preferred mspaceex for this thread */
	threadcache *tc;
	int mymspaceex, isforeign=1;
	size_t memsize;
	...
	GetThreadCache(&p, &tc, &mymspaceex, 0);
#if THREADCACHEMAX
	...
	LogOperation(tc, p, LOGENTRY_FREE, mymspaceex, memsize, mem, 0, 0, 0);
}
...
struct nedmallinfo nedpmallinfo(nedpool *p) THROWSPEC
{
	...
	for(n=0; p->m[n]; n++)
	{
		...
		struct mallinfo t=mspaceex_mallinfo(p->m[n]);
		ret.arena+=t.arena;
...
{
#if USE_ALLOCATOR==1
	return mspaceex_mallopt(parno, value);
#else
	return 0;
#endif
}
...
#elif USE_ALLOCATOR==1
    ret=mspaceex_independent_calloc(m, elemsno, elemsize, chunks);
#ifdef HAVE_VALGRIND
    if(ret)
    {
        size_t n;
...
NEDMALLOCNOALIASATTR NEDMALLOCPTRATTR void **nedpindependent_calloc(nedpool *p, size_t elemsno, size_t elemsize, void **chunks) THROWSPEC
{
	...
	threadcache *tc;
	int mymspaceex;
	GetThreadCache(&p, &tc, &mymspaceex, &elemsize);
	GETmspaceex(m, p, tc, mymspaceex, elemsno*elemsize,
	...
	if(ret && (ENABLE_LOGGING & LOGENTRY_POOL_MALLOC))
	{
		...
		for(n=0; n<elemsno; n++)
		{
			LogOperation(tc, p, LOGENTRY_POOL_MALLOC, mymspaceex, elemsize, 0, 0, M2_ZERO_MEMORY, ret[n]);
		}
	}
#endif
	return ret;
...
#elif USE_ALLOCATOR==1
	ret=mspaceex_independent_comalloc(m, elems, sizes, chunks);
#ifdef HAVE_VALGRIND
	if(ret)
	{
		size_t n;
...
NEDMALLOCNOALIASATTR NEDMALLOCPTRATTR void **nedpindependent_comalloc(nedpool *p, size_t elems, size_t *sizes, void **chunks) THROWSPEC
{
	...
	GetThreadCache(&p, &tc, &mymspaceex, 0);
	GETmspaceex(m, p, tc, mymspaceex, 0,
	...
	if(ret && (ENABLE_LOGGING & LOGENTRY_POOL_MALLOC))
	{
		...
		for(n=0; n<elems; n++)
		{
			LogOperation(tc, p, LOGENTRY_POOL_MALLOC, mymspaceex, sizes[n], 0, 0, 0, ret[n]);
		}
	}
#endif
	return ret;
