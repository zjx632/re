...
* Vital statistics:
  ...
  Security: static-safe; optionally more or less
       ...
       FOOTERS may also be worth using with mspaceexS.)
...
* mspaceexS
  If mspaceexS is defined, then in addition to malloc, free, etc.,
  this file also defines mspaceex_malloc, mspaceex_free, etc. These
  are versions of malloc routines that take an "mspaceex" argument
  obtained using create_mspaceex, to control all internal bookkeeping.
  If ONLY_mspaceexS is defined, only these versions are compiled.
  So if you would like to use this allocator for only some allocations,
  and your system malloc for others, you can compile with
  ONLY_mspaceexS and then do something like...
    static mspaceex mymspaceex = create_mspaceex(0,0); // for example
    #define mymalloc(bytes)  mspaceex_malloc(mymspaceex, bytes)

  (Note: If you only need one instance of an mspaceex, you can instead
  use "USE_DL_PREFIX" to relabel the global malloc.)

  You can similarly create thread-local allocators by storing
  mspaceexs as thread-locals. For example:
    static __thread mspaceex tlms = 0;
    void*  tlmalloc(size_t bytes) {
      if (tlms == 0) tlms = create_mspaceex(0, 0);
      return mspaceex_malloc(tlms, bytes);
    }
    void  tlfree(void* mem) { mspaceex_free(tlms, mem); }

  Unless FOOTERS is defined, each mspaceex is completely independent.
  You cannot allocate from one and free to another (although
  conformance is only weakly checked, so usage errors are not always
  caught). If FOOTERS is defined, then each chunk carries around a tag
  indicating its originating mspaceex, and frees are directed to their
  originating spaces.
...
mspaceexS                  default: 0 (false)
  If true, compile in support for independent allocation spaces.
  This is only supported if HAVE_MMAP is true.

ONLY_mspaceexS             default: 0 (false)
  If true, only compile in mspaceex versions, not regular versions.

USE_LOCKS                default: 0 (false)
  Causes each call to each public routine to be surrounded with
  pthread or WIN32 mutex lock/unlock. (If set true, this can be
  overridden on a per-mspaceex basis for mspaceex versions.) If set to a
...
HAVE_MORECORE             default: 1 (true) unless win32 or ONLY_mspaceexS
...
MAX_RELEASE_CHECK_RATE   default: 4095 unless not HAVE_MMAP
  ...
  especially with multiple mspaceexs, checking only for topmost space
...
#ifndef ONLY_mspaceexS
#define ONLY_mspaceexS 0     /* define to a value */
#else
#define ONLY_mspaceexS 1
#endif  /* ONLY_mspaceexS */
#ifndef mspaceexS
#if ONLY_mspaceexS
#define mspaceexS 1
#else   /* ONLY_mspaceexS */
...
/*
  mspaceex is an opaque type representing an independent
  region of space that supports mspaceex_malloc, etc.
*/
typedef void* mspaceex;

/*
  create_mspaceex creates and returns a new independent space with the
  given initial capacity, or, if 0, the default granularity size.  It
  returns null if there is no system memory available to create the
  space.  If argument locked is non-zero, the space uses a separate
  lock to control access. The capacity of the space will grow
  dynamically as needed to service mspaceex_malloc requests.  You can
  control the sizes of incremental increases of this space by
  compiling with a different DEFAULT_GRANULARITY or dynamically
  setting with mallopt(M_GRANULARITY, value).
*/
mspaceex create_mspaceex(size_t capacity, int locked);

/*
  destroy_mspaceex destroys the given space, and attempts to return all
  of its memory back to the system, returning the total number of
  bytes freed. After destruction, the results of access to all memory
  used by the space become undefined.
*/
size_t destroy_mspaceex(mspaceex msp);

/*
  create_mspaceex_with_base uses the memory supplied as the initial base
  of a new mspaceex. Part (less than 128*sizeof(size_t) bytes) of this
...
*/
mspaceex create_mspaceex_with_base(void* base, size_t capacity, int locked);

/*
  mspaceex_track_large_chunks controls whether requests for large chunks
  are allocated in their own untracked mmapped regions, separate from
  others in this mspaceex. By default large chunks are not tracked,
  which reduces fragmentation. However, such chunks are not
  necessarily released to the system upon destroy_mspaceex.  Enabling
  tracking by setting to true may increase fragmentation, but avoids
  leakage when relying on destroy_mspaceex to release all memory
  allocated using this space.  The function returns the previous
  setting.
*/
int mspaceex_track_large_chunks(mspaceex msp, int enable);


/*
  mspaceex_malloc behaves as malloc, but operates within
  the given space.
*/
void* mspaceex_malloc(mspaceex msp, size_t bytes);

/*
  mspaceex_free behaves as free, but operates within
  the given space.

  If compiled with FOOTERS==1, mspaceex_free is not actually needed.
  free may be called instead of mspaceex_free because freed chunks from
  any space are handled by their originating spaces.
*/
void mspaceex_free(mspaceex msp, void* mem);

/*
  mspaceex_realloc behaves as realloc, but operates within
  the given space.

  If compiled with FOOTERS==1, mspaceex_realloc is not actually
  needed.  realloc may be called instead of mspaceex_realloc because
  realloced chunks from any space are handled by their originating
  spaces.
*/
void* mspaceex_realloc(mspaceex msp, void* mem, size_t newsize);

/*
  mspaceex_calloc behaves as calloc, but operates within
  the given space.
*/
void* mspaceex_calloc(mspaceex msp, size_t n_elements, size_t elem_size);

/*
  mspaceex_memalign behaves as memalign, but operates within
  the given space.
*/
void* mspaceex_memalign(mspaceex msp, size_t alignment, size_t bytes);

/*
  mspaceex_independent_calloc behaves as independent_calloc, but
  operates within the given space.
*/
void** mspaceex_independent_calloc(mspaceex msp, size_t n_elements,
                                 size_t elem_size, void* chunks[]);

/*
  mspaceex_independent_comalloc behaves as independent_comalloc, but
  operates within the given space.
*/
void** mspaceex_independent_comalloc(mspaceex msp, size_t n_elements,
                                   size_t sizes[], void* chunks[]);

/*
  mspaceex_malloc2 behaves as mspaceex_malloc, but provides additional
  functionality. Setting alignment to a non-zero value is
  identical to using mspaceex_memalign(). Flags may be set to:
...
*/
void* mspaceex_malloc2(mspaceex msp, size_t bytes, size_t alignment, unsigned flags);

/*
  mspaceex_realloc2 behaves as mspaceex_realloc, but provides additional
  functionality. Setting alignment to a non-zero value is
  identical to using mspaceex_memalign(). Flags may be set to:
...
*/
void* mspaceex_realloc2(mspaceex msp, void* mem, size_t newsize, size_t alignment, unsigned flags);

/*
  mspaceex_footprint() returns the number of bytes obtained from the
  system for this space.
*/
size_t mspaceex_footprint(mspaceex msp);

/*
  mspaceex_max_footprint() returns the peak number of bytes obtained from the
  system for this space.
*/
size_t mspaceex_max_footprint(mspaceex msp);


#if !NO_MALLINFO
/*
  mspaceex_mallinfo behaves as mallinfo, but reports properties of
  the given space.
*/
struct mallinfo mspaceex_mallinfo(mspaceex msp);
#endif /* NO_MALLINFO */

/*
  malloc_usable_size(void* p) behaves the same as malloc_usable_size;
*/
  size_t mspaceex_usable_size(void* mem);

/*
  mspaceex_malloc_stats behaves as malloc_stats, but reports
  properties of the given space.
*/
void mspaceex_malloc_stats(mspaceex msp);

/*
  mspaceex_trim behaves as malloc_trim, but
  operates within the given space.
*/
int mspaceex_trim(mspaceex msp, size_t pad);

/*
  An alias for mallopt.
*/
int mspaceex_mallopt(int, int);
...
/* segment bit set in create_mspaceex_with_base */
...
/*
  ...
  Per-mspaceex locks surround calls to malloc, free, etc.  To enable use
  in layered extensions, per-mspaceex locks are reentrant.
...
/*
  ...
  * If EXTERN_BIT set, then we did not allocate this segment,
    ...
    into create_mspaceex_with_base.)
...
/*
  ...
  Locking
    ...
    around every public call using this mspaceex.
...
/* The global malloc_state used for all non-"mspaceex" calls */
...
#if ONLY_mspaceexS
static void* mspaceex_malloc_implementation(mstate ms, size_t bytes, unsigned flags);
#define internal_malloc(m, b, f) mspaceex_malloc_implementation(m, b, f)
#define internal_free(m, mem) mspaceex_free(m,mem);
#else /* ONLY_mspaceexS */
#if mspaceexS
static void* mspaceex_malloc_implementation(mstate ms, size_t bytes, unsigned flags);
#define internal_malloc(m, b, f)\
   (m == gm)? dlmalloc(b) : mspaceex_malloc_implementation(m, b, f)
#define internal_free(m, mem)\
   if (m == gm) dlfree(mem); else mspaceex_free(m,mem);
#else /* mspaceexS */
...
/*
  mspaceex versions of routines are near-clones of the global
  versions. This is not so nice but better than the alternatives.
*/

static FORCEINLINE void* mspaceex_malloc_implementation(mstate ms, size_t bytes, unsigned flags) {
  if (!PREACTION(ms)) {
    void* mem;
    size_t nb = bytes;
...
void* mspaceex_malloc(mspaceex msp, size_t bytes) {
  ...
  return mspaceex_malloc_implementation(ms, bytes, 0);
}
void* mspaceex_malloc2(mspaceex msp, size_t bytes, size_t alignment, unsigned flags) {
  void* mem;
  mstate ms = (mstate)msp;
  ...
  if (alignment <= MALLOC_ALIGNMENT)
    mem = mspaceex_malloc_implementation(ms, bytes, flags);
  else
...
void* mspaceex_realloc2(mspaceex msp, void* oldmem, size_t bytes, size_t alignment, unsigned flags) {
  if (oldmem == 0)
    return mspaceex_malloc2(msp, bytes, alignment, flags);
#ifdef REALLOC_ZERO_BYTES_FREES
  if (bytes == 0) {
    mspaceex_free(msp, oldmem);
    return 0;
  }
#endif /* REALLOC_ZERO_BYTES_FREES */
  else {
...
void* mspaceex_realloc(mspaceex msp, void* oldmem, size_t bytes) {
  return mspaceex_realloc2(msp, oldmem, bytes, 0, 0);
}
...
int mspaceex_mallopt(int param_number, int value) {
  return change_mparam(param_number, value);
}
...
History:
    V2.8.4 Wed May 27 09:56:23 2009  Doug Lea  (dl at gee)
      ...
      * Add mspaceex_track_large_chunks; thanks to Jean Brouwers
      * Fix set_inuse in internal_realloc; thanks to Jean Brouwers
      * Fix insufficient sys_alloc padding when using 16byte alignment
      * Fix bad error check in mspaceex_footprint
    ...
    V2.8.3 Thu Sep 22 11:16:32 2005  Doug Lea  (dl at gee)
      ...
      * Avoid concatenating segments with the one provided
        in create_mspaceex_with_base
      * Rename some variables to avoid compiler shadowing warnings
      * Use explicit lock initialization.
      * Better handling of sbrk interference.
      * Simplify and fix segment insertion, trimming and mspaceex_destroy
